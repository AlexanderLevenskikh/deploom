...
Так появилась DepLoom — настольная программа, которая превращает разрозненное обновление зависимостей в понятный и воспроизводимый процесс.
...
DepLoom собирает этот процесс в одну последовательность из девяти этапов:
...
**Проверяет результат, а не только код завершения.** Успешное окончание процесса агента ещё не означает успешную миграцию. DepLoom сверяет реальные ветки с планом, проверяет, что изменения дошли до объединённой ветки, что рабочая папка чистая и что обновлённый отчёт подтверждает результат.
...
Откройте страницу [последнего выпуска DepLoom](https://github.com/AlexanderLevenskikh/deploom/releases/latest), скачайте установщик `.exe` из приложенных файлов и запустите его.
...
DepLoom сознательно остаётся помощником, а не автономной системой выпуска.
...
## Draft-режим и целевая политика

**Draft — это planning-only результат.** DepLoom может быстро построить черновик плана без physical install/lifecycle/project-проверок: `verificationStatus = NOT_VERIFIED`, `authority = PLANNING_ONLY`, `compatibility = UNKNOWN`. Такой результат публикуется отдельно от verified-артефактов (только под `.dependency-roadmap/artifacts/runs/<runId>/draft/`) и никогда не перезаписывает проверенный roadmap или доказательства, поэтому его нельзя спутать с подтверждённой миграцией. Если в реестре нет свежих метаданных, для таких пакетов явно указывается «неизвестно», а не выдумываются безопасные target или «0 уязвимостей».

**Цель запуска настраивается.** В диалоге «Состав Baseline» выбираются целевой уровень (Жёлтый/Зелёный) и минимум актуальности (сколько процентов библиотек должны соблюдать lag-policy, по умолчанию 80). Эти значения входят в versioned policy (policy hash) и реально влияют на пороги плана и приёмки: Жёлтый требует заданный процент актуальных библиотек, Зелёный — на 10 пунктов строже, план держит запас ещё +5 пунктов. Пороги и цель явно записаны в Draft-промпте. Partial-результат сам по себе никогда не считается достижением цели.

Детали ревалидации и доказательства (R1–R10) — в `docs/DRAFT_MODES_REVALIDATION_REPORT_2026-09-23.md`.