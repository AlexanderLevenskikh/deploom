...
|---|---|---|---|
| C2 | P1 | Draft-прогон для реального 76-пакетного проекта (run-5fadef80) вернул «registry metadata unavailable» для всех 77 пакетов и пустой prompt, хотя Nexus доступен. Разобраться и починить так, чтобы при доступном реестре Draft реально читал метаданные и строил план | закрыта |
...
Корпоративный Nexus отвечает ~1.1с на документ ~7МБ. Прокси не виноваты: `--use-system-proxy` выключен по умолчанию (`trust_env=False`), но прямой доступ работает.
