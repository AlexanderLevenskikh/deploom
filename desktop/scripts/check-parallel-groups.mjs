...
for (const required of ['liveMaxParallelGroups', 'configured slots=${maxParallelGroups}', 'parallelWorkerJob(parent, branch)', "'worktree', 'prune', '--expire', 'now'", 'jobId: worker.job.id', 'Причина: ${message.slice(0, 3000)}', 'openCodeTransportOwner(job)', 'if (owner !== job) return', 'client-${randomUUID()}', 'OPENCODE_SERVER_START_ATTEMPTS = 3', 'if (owner.openCodeServerStarting) return owner.openCodeServerStarting', 'ensureMigrationBaseBranch(job, project, initialPlan, savedPromptMarkdown)', 'MIGRATION_BASE_SOURCE_COMMIT_MISSING', "'branch', baseBranch, sourceHead"]) if (!mainSource.includes(required)) throw new Error(`parallel runtime contract missing: ${required}`)
