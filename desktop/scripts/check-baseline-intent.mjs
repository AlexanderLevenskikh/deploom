...
mustContain(preload, "ipcRenderer.invoke('flow:current-project-prompt-preview')", 'prompt preview must delegate to main process')
mustContain(main, "ipcMain.handle('flow:current-project-prompt-preview'", 'trusted prompt preview main-process handler')
mustContain(main, 'const targetPath = details.projectPromptPath', 'prompt preview path must come from trusted workspace state')
mustNotContain(preload, "node:fs", 'sandboxed preload must not access the filesystem directly')
...
mustContain(flowWorkspace, "text('Начать заново', 'Start over')", 'Baseline restart must stay visible next to the human-flow action')
