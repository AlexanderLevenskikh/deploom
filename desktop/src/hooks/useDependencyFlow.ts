...
export function useDependencyFlow() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      if (!api && !DEMO_MODE) throw new Error('DESKTOP_BRIDGE_UNAVAILABLE: Electron preload bridge did not initialize. DepLoom will not silently fall back to demo data.')
