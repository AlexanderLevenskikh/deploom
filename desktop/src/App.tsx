...
function App() {
  ...
  if (!payload) {
    return <div className="splash"><Workflow size={28} /><strong>DepLoom</strong><span>{flow.error || 'Desktop bridge is unavailable.'}</span></div>
  }
  ...
  return (
    <div className={`app-shell${graphActive && graphFullscreen ? ' graph-fullscreen' : ''}`}>
      ...
      <div className={`app-grid${graphHideLeft ? ' graph-hide-project-rail' : ''}`}>
        ...
        <main className="main-workspace">
          ...
          {tab === 'flow' && project ? <FlowWorkspace details={details} project={project} activeAction={flow.activeWorkspaceId === details.workspace.id && flow.activeProjectName === project.name ? flow.activeAction : undefined} activeRunId={flow.activeRunId} activeRunStartedAt={flow.activeRunStartedAt} activeDraftProgress={flow.activeDraftProgress} draftLaunch={flow.draftLaunch} onMarkDraftLaunched={flow.markDraftLaunched} onResetDraftLaunch={flow.resetDraftLaunch} onAcknowledgeDraftRun={flow.acknowledgeDraftRun} autopilotActive={flow.autopilotActive && flow.autopilotProjectName === project.name} baselineDecision={flow.baselineDecision} onClearBaselineDecision={flow.clearBaselineDecision} onGetBaselineIntentPlan={flow.getBaselineIntentPlan} onGetCurrentDraftResult={flow.getCurrentDraftResult} onRun={flow.runAction} onSendAgentNote={flow.sendAgentNote} onStartAutopilot={flow.startAutopilot} onStopAutopilot={flow.stopAutopilot} onRecoverWithAgent={flow.recoverWithAgent} onOpenDashboard={openDashboard} onOpenPath={flow.openPath} onChoosePrompt={flow.choosePrompt} onUpdateWorkspace={flow.updateWorkspace} onUpdateProjectBranches={flow.updateProjectBranches} onListAgentModels={flow.listAgentModels} /> : null}
