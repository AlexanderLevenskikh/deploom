...
function nonRetryableDeterministicFailure(result: { code: number; stderr: string; stdout: string }): boolean {
  ...
  return /BASELINE_VERIFY_UNKNOWN_ERROR|BASELINE_VERIFY_INCONCLUSIVE_PROJECT_ERROR|BASELINE_PLAN_BROKEN|BASELINE_CONSTRAINT_LOOP_STUCK|BASELINE_CONSTRAINT_BUDGET_EXHAUSTED|BASELINE_VERIFICATION_PLATEAU|BASELINE_VERIFICATION_HARD_BUDGET_EXHAUSTED|BASELINE_SOLVER_REPEATED_FAILED_ASSIGNMENT|BASELINE_CONSTRAINT_MINIMIZATION_INCONCLUSIVE|BASELINE_NOOP_RESOLVER_INVALID|EXACT_SOLVER_PROOF_REQUIRED|FIXED_INPUT_CONSTRAINT_CONFLICT|HETEROGENEOUS_DIRECT_DEPENDENCY_DECLARATION|FIXED_DEPENDENCY_DECLARATION_MISMATCH|ASSIGNMENT_HETEROGENEOUS_SOURCE_CONFLICT|ASSIGNMENT_TARGETS_FIXED_INPUT|ASSIGNMENT_REMOVES_FIXED_INPUT|PROVEN_ASSIGNMENT_REOPENED|PROVEN_ASSIGNMENT_MUTATED|PROVEN_DEPENDENCY_ENVELOPE_INVALID|FINAL_BASELINE_COMPATIBILITY_INVALID|OBSERVED_RESOLVED_ASSIGNMENT_[A-Z0-9_]+/i.test(text)
}
