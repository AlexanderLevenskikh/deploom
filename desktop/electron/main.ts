...
function setupIpc(): void {
  ...
  ipcMain.handle('flow:current-project-prompt-preview', async () => {
    // Keep filesystem authority in the main process. The renderer supplies no
    // path: the current prompt is resolved from trusted workspace state only.
    const state = loadState()
    const workspace = findWorkspace(state)
    const details = await workspaceDetails(workspace)
