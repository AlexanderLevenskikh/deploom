...
function replaceLegacyScopeSaveRule(prompt: string, scopeManifestPath: string): string {
  ...
  const replacement = `1. DepLoom already materialized this exact compact manifest at \`${scopePath}\`. Do not create another *_scope.json for this batch. Verify the file/hash before edits and reference this exact path from checkpoint evidence.`
  return prompt
    .replace(
    ...
    .replace(
      /1\. When run through DepLoom Desktop,[^\n]+validator supports `compact-v1`\./,
      replacement,
    )
}
...
export function injectExecutorBatchContext(prompt: string, context: ExecutorBatchContext): string {
  ...
  const contract = `## DepLoom executor contract — authoritative\n\n` +
    ...
    `- Baseline facts produced by DepLoom solve-and-verify are authoritative when present in release/preflight evidence. Reuse them in checkpoint evidence instead of rerunning the same baseline checks. node_modules being absent in a Git worktree is expected.\n` +
